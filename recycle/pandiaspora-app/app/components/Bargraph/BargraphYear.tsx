...
const BargraphYear = (props: BargraphYearProps) => {
  useEffect(() => {
    draw()
  })
  ...
  const draw = () => {
    ...
    d3.dsv(',', props.file, (d) => {
      ...
      y.domain([
        0,
        d3.max(data, (d) => {
          return Math.max(...data.map((dt) => (dt as Types.Data).y), 0) + 100
        }),
      ] as number[])
  ...
  return <div className="barGraphYear" />
}

interface BargraphYearProps {
  width: number
  height: number
  top: number
  right: number
